...
async def get_total_revenue():
    with engine.connect() as conn:
        stmt = text("SELECT total_revenue FROM total_revenue")  # Usa a função text() para executar a consulta
...
async def get_sales_per_month():
    with engine.connect() as conn:
        ...
        if results:
            return [{"sales_month": row[0], "sales_per_month": row[1]} for row in results]
        else:
            raise HTTPException(status_code=404, detail="KPI não encontrada")
...
async def get_revenue_per_month():
    with engine.connect() as conn:
        ...
        if results:
            return [{"revenue_month": row[0], "revenue_per_month": row[1]} for row in results]
        else:
            raise HTTPException(status_code=404, detail="KPI não encontrada")
