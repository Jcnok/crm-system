...
async def get_total_revenue(ano: int):
    with engine.connect() as conn:
        stmt = text("SELECT * FROM total_revenue WHERE year = :ano")
...
async def get_product_revenue(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM product_revenue WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
...
async def get_sales_per_salesperson(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM sales_per_salesperson WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
            return [
...
async def get_revenue_per_salesperson(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM revenue_per_salesperson WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
            return [
...
async def get_sales_per_day(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM sales_per_day WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
            return [
...
async def get_sales_per_month(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM sales_per_month WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
            return [
...
@app.get("/sales_per_year")  # Remove o parâmetro ano do path, adiciona como query param
async def get_sales_per_year(ano: int = None):
    with engine.connect() as conn:
        if ano:
            results = conn.execute(
                text("SELECT * FROM sales_per_year WHERE sales_year = :ano"),
                {"ano": ano},
            ).fetchall()
        else:
            results = conn.execute(
                text("SELECT * FROM sales_per_year")
            ).fetchall()
        if results:
...
async def get_revenue_per_day(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM revenue_per_day WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
            return [
...
async def get_revenue_per_month(ano: int):
    with engine.connect() as conn:
        results = conn.execute(
            text("SELECT * FROM revenue_per_month WHERE year = :ano"),
            {"ano": ano},
        ).fetchall()
        if results:
            return [
...
@app.get("/revenue_per_year")  # Remove o parâmetro ano do path, adiciona como query param
async def get_revenue_per_year(ano: int = None):
    with engine.connect() as conn:
        if ano:
            results = conn.execute(
                text("SELECT * FROM revenue_per_year WHERE revenue_year = :ano"),
                {"ano": ano},
            ).fetchall()
        else:
            results = conn.execute(
                text("SELECT * FROM revenue_per_year")
            ).fetchall()
        if results:
            return [
